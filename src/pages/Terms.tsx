...
export default function Terms() {
  useEffect(() => {
    ...
    document.title = TERMS_PAGE_TITLE;
    ...
    description.content = TERMS_META_DESCRIPTION;
    ...
    <div className="sos-page page-atmosphere page-nav-offset mx-auto max-w-3xl px-6 pb-28 sm:px-8">
      <header className="sos-page__header">
        ...
        <h1 className="sos-page__title">
          <span className="sos-page__title-route">/terms</span> of service
        </h1>
        <p className="sos-page__subtitle">
          The agreement between you and {TERMS_ENTITY} for USJET.AI. Effective {TERMS_EFFECTIVE_DATE}. Canonical URL:{" "}
          <a href={TERMS_CANONICAL_URL} className="sos-page__inline-link">
            {TERMS_CANONICAL_URL}
          </a>
          .
        </p>
                ...
                <p key={paragraph.slice(0, 64)} className="sos-page__lead">
                  {paragraph.includes("ops@usjet.ai") ? (
                    <>
                      ...
                      <a href={mailtoUsjetOps("USJET Terms of Service")} className="sos-page__inline-link">
                        {USJET_OPS_EMAIL}
                      </a>
      ...
      <section className="sos-page__footer mt-12 text-center" aria-labelledby="terms-footer-heading">
        <p id="terms-footer-heading" className="sos-page__subtitle mx-auto mb-5 max-w-md text-balance">
          Questions on billing, orders, or your clearance tier? Start with site operating support.
        </p>
        <Link
          to="/sos"
        ...
        >
          <FileText className="sos-page__ai101-badge__icon" size={20} strokeWidth={2.2} aria-hidden />
