...
export default function Privacy() {
  useEffect(() => {
    ...
    document.title = PRIVACY_PAGE_TITLE;
    ...
    description.content = PRIVACY_META_DESCRIPTION;
    ...
    <div className="sos-page page-atmosphere page-nav-offset mx-auto max-w-3xl px-6 pb-28 sm:px-8">
      <header className="sos-page__header">
        ...
        <h1 className="sos-page__title">
          <span className="sos-page__title-route">/privacy</span> policy
        </h1>
        <p className="sos-page__subtitle">
          How {PRIVACY_ENTITY} handles information on USJET.AI. Effective {PRIVACY_EFFECTIVE_DATE}. Canonical URL:{" "}
          <a href={PRIVACY_CANONICAL_URL} className="sos-page__inline-link">
            {PRIVACY_CANONICAL_URL}
          </a>
          .
        </p>
          ...
          {PRIVACY_SECTIONS.map((section) => (
            <section key={section.id} id={section.id} className="sos-page__section">
              ...
              {section.paragraphs.map((paragraph) => (
                <p key={paragraph} className="sos-page__lead">
                  {paragraph.includes("ops@usjet.ai") ? (
                    <>
                      {paragraph.split("ops@usjet.ai")[0]}
                      <a href={mailtoUsjetOps("USJET Privacy Policy")} className="sos-page__inline-link">
                        {USJET_OPS_EMAIL}
                      </a>
                      {paragraph.split("ops@usjet.ai")[1]}
                    </>
                  ) : paragraph.includes("/sos") ? (
                    <>
                      {paragraph.split("/sos")[0]}
                      <Link to="/sos" className="sos-page__inline-link">
                        /sos
                      </Link>
                      {paragraph.split("/sos")[1]}
                    </>
                  ) : paragraph.includes("/cockpit") ? (
                    <>
                      {paragraph.split("/cockpit")[0]}
                      <Link to="/cockpit" className="sos-page__inline-link">
                        /cockpit
                      </Link>
                      {paragraph.split("/cockpit")[1]}
                    </>
                  ) : (
                    paragraph
                  )}
      ...
      <section className="sos-page__footer mt-12 text-center" aria-labelledby="privacy-footer-heading">
        <p id="privacy-footer-heading" className="sos-page__subtitle mx-auto mb-5 max-w-md text-balance">
          Need a line check on cache, mic, or member ID? Start with site operating support.
        </p>
        <Link
          to="/sos"
        ...
        >
          <Shield className="sos-page__ai101-badge__icon" size={20} strokeWidth={2.2} aria-hidden />
