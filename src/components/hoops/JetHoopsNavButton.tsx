  ...
  if (variant === "chip") {
    return (
      <NavLink
        to={JET_HOOPS_ROUTE}
        className={({ isActive }) =>
          ["app-nav-hoops btn-glass glass-effect-interactive shrink-0", isActive ? "app-nav-hoops--active" : ""]
            .filter(Boolean)
            .join(" ")
        }
      ...
      >
        <span className="app-nav-hoops__glow" aria-hidden />
        <span className="app-nav-hoops__ball" aria-hidden />
        <span className="app-nav-hoops__label">Hoops</span>
      </NavLink>
    );
  }
